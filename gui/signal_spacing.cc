...
{
	settings_t setting;
	uint16 meters_per_tile=welt->get_settings().get_meters_per_tile();
	player = player_;
	tool = tool_;
	...
	signal_spacing_inp.set_width_by_len(6);
	...
	sprintf(str_meter_label,"%d m", signal_spacing*welt->get_settings().get_meters_per_tile());
	meter_label.set_text_pointer(str_meter_label, false);
	meter_label.set_pos( scr_coord(L_DIALOG_WIDTH - D_MARGIN_RIGHT - signal_spacing_inp.get_size().w+12, cursor.y) );
	meter_label.set_width( 50 );
	add_component( &meter_label );

	cursor.y += meter_label.get_size().h + D_V_SPACE;
