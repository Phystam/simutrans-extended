...
gui_convoy_assembler_t::gui_convoy_assembler_t(waytype_t wt, signed char player_nr, bool electrified) :
	...
	lb_too_heavy_notice("too heavy", color_idx_to_rgb(COL_RED), gui_label_t::left),
...
void gui_convoy_assembler_t::image_from_storage_list(gui_image_list_t::image_data_t* image_data)
{
	...
	if(depot_frame)
	{
		...
		if(image_data->lcolor !=  color_idx_to_rgb(COL_RED) &&
			 image_data->rcolor !=  color_idx_to_rgb(COL_RED) &&
			 image_data->rcolor !=  color_idx_to_rgb(COL_GREY3) &&
			 image_data->lcolor !=  color_idx_to_rgb(COL_GREY3) &&
			 image_data->rcolor !=  color_idx_to_rgb(COL_DARK_PURPLE) &&
			 image_data->lcolor !=  color_idx_to_rgb(COL_DARK_PURPLE) &&
			 image_data->rcolor !=  color_idx_to_rgb(COL_UPGRADEABLE) &&
			 image_data->lcolor !=  color_idx_to_rgb(COL_UPGRADEABLE) &&
			 !((image_data->lcolor == color_idx_to_rgb(COL_DARK_ORANGE) || image_data->rcolor == color_idx_to_rgb(COL_DARK_ORANGE))
			&& veh_action != va_sell
...
void gui_convoy_assembler_t::update_data()
{
	...
	if(vehicles.get_count() > 0) {
		...
		convoi_pics[0]->lcolor = color_idx_to_rgb(vehicles[0]->can_follow(NULL) ? COL_DARK_GREEN : COL_YELLOW);
		...
		for(  i=1;  i<vehicles.get_count(); i++) {
			convoi_pics[i - 1]->rcolor = color_idx_to_rgb(vehicles[i - 1]->can_lead(vehicles[i]) ? COL_DARK_GREEN : COL_RED);
			convoi_pics[i]->lcolor     = color_idx_to_rgb(vehicles[i]->can_follow(vehicles[i - 1]) ? COL_DARK_GREEN : COL_RED);
		...
		convoi_pics[i - 1]->rcolor = color_idx_to_rgb(vehicles[i - 1]->can_lead(NULL) ? COL_DARK_GREEN : COL_YELLOW);

		// change green into blue for retired vehicles
		for(i=0;  i<vehicles.get_count(); i++) {
			if(vehicles[i]->is_future(month_now) || vehicles[i]->is_retired(month_now)) {
				if (convoi_pics[i]->lcolor == color_idx_to_rgb(COL_DARK_GREEN)) {
					if (vehicles[i]->is_obsolete(month_now, welt)) {
						convoi_pics[i]->lcolor = color_idx_to_rgb(COL_OBSOLETE);
					}
					else {
						convoi_pics[i]->lcolor = color_idx_to_rgb(COL_OUT_OF_PRODUCTION);
					}
				}
				if (convoi_pics[i]->rcolor == color_idx_to_rgb(COL_DARK_GREEN)) {
					if (vehicles[i]->is_obsolete(month_now, welt)) {
						convoi_pics[i]->rcolor = color_idx_to_rgb(COL_OBSOLETE);
					}
					else {
						convoi_pics[i]->rcolor = color_idx_to_rgb(COL_OUT_OF_PRODUCTION);
					}
				}
			}
		}
	...
	FOR(vehicle_image_map, const& i, vehicle_map) 
	{
		...
		PIXVAL ok_color = color_idx_to_rgb(info->is_future(month_now) || info->is_retired(month_now) ? COL_OUT_OF_PRODUCTION : COL_DARK_GREEN);
		if (info->is_obsolete(month_now, welt)) {
			ok_color = color_idx_to_rgb(COL_OBSOLETE);
		}
		...
		*/
		...
		if (veh_action != va_sell)
		{
			...
			// @author: jamespetts
			if(img.lcolor == ok_color || img.lcolor == color_idx_to_rgb(COL_YELLOW))
			{
				...
				if(veh_action != va_upgrade)
				{
					...
					{
						img.lcolor = color_idx_to_rgb(COL_DARK_ORANGE);
						img.rcolor = color_idx_to_rgb(COL_DARK_ORANGE);
					}
				}
				else
				{
					...
					
				{
					...
					img.lcolor = color_idx_to_rgb(COL_GREY3);
					img.rcolor = color_idx_to_rgb(COL_GREY3);
				}
			}
		}
		else
			...
			ITERATE(vehicle_list, i)
			{
				for(uint16 c = 0; c < vehicle_list[i]->get_upgrades_count(); c++)
				{
					if(vehicle_list[i]->get_upgrades(c) && (info == vehicle_list[i]->get_upgrades(c)))
					{
						...
						{
							img.lcolor = color_idx_to_rgb(COL_DARK_GREEN);
							img.rcolor = color_idx_to_rgb(COL_DARK_GREEN);
						}
						if(replace_frame != NULL)
						{
		...
		{	
			if(info->is_available_only_as_upgrade())
			{
				...
				if(purple)
				{
					img.lcolor = color_idx_to_rgb(COL_UPGRADEABLE);
					img.rcolor = color_idx_to_rgb(COL_UPGRADEABLE);
				}
			}
		}
	...
	if (depot_frame) 
	{
		FOR(slist_tpl<vehicle_t *>, const& iter2, depot_frame->get_depot()->get_vehicle_list())
		{
			...
			if(imgdat) 
			{
				...
				if(veh_action == va_sell) 
				{
					imgdat->lcolor = color_idx_to_rgb(COL_DARK_GREEN);
					imgdat->rcolor = color_idx_to_rgb(COL_DARK_GREEN);
				}
			}
		}
...
void gui_convoy_assembler_t::draw_vehicle_info_text(const scr_coord& pos)
{
	...
	if ((sel_index != -1) && (tabs.getroffen(x-pos.x,y-pos.y))) {
		...
		if(vec[sel_index]->lcolor == color_idx_to_rgb(COL_GREY3))
		{
		...
		if (veh_action == va_upgrade && vec[sel_index]->lcolor == color_idx_to_rgb(COL_DARK_GREEN)) {
			convoihandle_t cnv;
			if (depot_frame)
			{
				cnv = depot_frame->get_convoy();
		...
		if(sel_index != -1) {
			if (depot_frame) 
			{
				...
				if(cnv.is_bound())
				{
					...
					lb_convoi_number.set_color(color_idx_to_rgb(veh_type->has_available_upgrade(welt->get_timeline_year_month(), welt->get_settings().get_show_future_vehicle_info()) == 2? COL_UPGRADEABLE : COL_WHITE));
...
//			top = offset.y + D_V_SPACE + grid.y;
//		}
//		display_fillbox_wh_clip_rgb(left, top+2, VEHICLE_BAR_HEIGHT*2, VEHICLE_BAR_HEIGHT, bar_colors[i], true);
...
//	left += padding + VEHICLE_BAR_HEIGHT * 2 + 3 + D_H_SPACE;
	...
	top += LINESPACE;
	left += D_MARGIN_LEFT;
	display_veh_form_wh_clip_rgb(left, top + 2, VEHICLE_BAR_HEIGHT*2, color_idx_to_rgb(COL_DARK_GREEN), true, vehicle_desc_t::can_be_head, 0, false);
	display_veh_form_wh_clip_rgb(left + VEHICLE_BAR_HEIGHT*2, top + 2, VEHICLE_BAR_HEIGHT*2, color_idx_to_rgb(COL_DARK_GREEN), true, vehicle_desc_t::can_be_tail, 0, true);
	...
	top += LINESPACE;
	display_veh_form_wh_clip_rgb(left, top + 2, VEHICLE_BAR_HEIGHT*2, color_idx_to_rgb(COL_DARK_GREEN), true, vehicle_desc_t::can_be_head, 1, false);
	display_veh_form_wh_clip_rgb(left + VEHICLE_BAR_HEIGHT*2, top + 2, VEHICLE_BAR_HEIGHT*2, color_idx_to_rgb(COL_DARK_GREEN), true, (vehicle_desc_t::can_be_head + vehicle_desc_t::can_be_tail), 1, true);
	...
	top += LINESPACE;
	display_veh_form_wh_clip_rgb(left, top + 2, VEHICLE_BAR_HEIGHT*2, color_idx_to_rgb(COL_DARK_GREEN), true, vehicle_desc_t::can_be_tail, 1, false);
	display_veh_form_wh_clip_rgb(left + VEHICLE_BAR_HEIGHT*2, top + 2, VEHICLE_BAR_HEIGHT*2, color_idx_to_rgb(COL_DARK_GREEN), true, vehicle_desc_t::can_be_tail, 1, true);
	...
	top += LINESPACE;
	display_veh_form_wh_clip_rgb(left, top + 2, VEHICLE_BAR_HEIGHT*2, color_idx_to_rgb(COL_DARK_GREEN), true, 0, 0, false);
	display_veh_form_wh_clip_rgb(left + VEHICLE_BAR_HEIGHT*2, top + 2, VEHICLE_BAR_HEIGHT*2, color_idx_to_rgb(COL_DARK_GREEN), true, 0, 0, true);
	...
	/*
	top += LINESPACE;
	display_veh_form_wh_clip_rgb(left, top + 2, VEHICLE_BAR_HEIGHT * 2, COL_DARK_GREEN, true, vehicle_desc_t::unknown_constraint, 0, false);
	display_veh_form_wh_clip_rgb(left + VEHICLE_BAR_HEIGHT * 2, top + 2, VEHICLE_BAR_HEIGHT * 2, COL_DARK_GREEN, true, vehicle_desc_t::unknown_constraint, 0, true);
	...
	top += LINESPACE;
	display_veh_form_wh_clip_rgb(left, top + 2, VEHICLE_BAR_HEIGHT*2, COL_DARK_GREEN, true, vehicle_desc_t::fixed_coupling_prev, 0, false, false);
	display_veh_form_wh_clip_rgb(left + VEHICLE_BAR_HEIGHT*2, top + 2, VEHICLE_BAR_HEIGHT*2, COL_DARK_GREEN, true, vehicle_desc_t::fixed_coupling_next, 0, true, false);
	...
	*/
	...
	display_fillbox_wh_clip_rgb(left+1, top+2, VEHICLE_BAR_HEIGHT*2 - 2, VEHICLE_BAR_HEIGHT, color_idx_to_rgb(COL_DARK_GREEN), true);
	...
	top += LINESPACE;
	display_fillbox_wh_clip_rgb(left+1, top + 2, VEHICLE_BAR_HEIGHT*2 - 2, VEHICLE_BAR_HEIGHT, color_idx_to_rgb(COL_DARK_GREEN), true);
	display_blend_wh_rgb(left+3, top + 3, VEHICLE_BAR_HEIGHT*2 - 6, VEHICLE_BAR_HEIGHT - 2, color_idx_to_rgb(COL_WHITE), 30);
	...
	display_veh_form_wh_clip_rgb(left, top + 2, VEHICLE_BAR_HEIGHT*2 - 1, COL_DARK_GREEN, true, (vehicle_desc_t::permanent_coupling_next + vehicle_desc_t::fixed_coupling_next), 0, true, false);
	display_veh_form_wh_clip_rgb(left + VEHICLE_BAR_HEIGHT*2 + 1, top + 2, VEHICLE_BAR_HEIGHT*2 - 1, COL_DARK_GREEN, true, (vehicle_desc_t::permanent_coupling_prev + vehicle_desc_t::fixed_coupling_prev), 0, false, false);
